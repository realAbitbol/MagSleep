...
- `make release VERSION=X.Y.Z` automates the whole release (`scripts/release.sh`): it refuses dirty trees and existing tags, runs tests + `make dmg`, uploads the DMG to **VirusTotal** (via `scripts/virustotal-scan.sh`, requires the `VIRUSTOTAL_API_KEY` env var; without it the scan is skipped with a warning) and writes the result as a README badge + CHANGELOG proof line, sed-updates the README `VERSION=1.0.x` examples and the Makefile default, commits, creates an annotated tag, pushes branch + tag to origin, and publishes the GitHub Release with the DMG + ZIP via `gh`. Notarization stays a separate manual step (`make notarize`).
