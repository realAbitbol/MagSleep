...
- `make release VERSION=X.Y.Z` automates the whole release (`scripts/release.sh`): it refuses dirty trees and existing tags, runs tests + `make dmg`, sed-updates the README `VERSION=1.0.x` examples and the Makefile default, commits, creates an annotated tag, pushes branch + tag to origin, and publishes the GitHub Release with the DMG + ZIP via `gh`. Notarization stays a separate manual step (`make notarize`).
